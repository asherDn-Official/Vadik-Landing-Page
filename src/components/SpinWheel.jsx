...
const SpinWheel = ({
  ...
  // 1. Identify targeted segment indexes
  ...
  segments.forEach((segment, index) => {
    ...
    if (
      segmentId &&
      targetedCoupons &&
      targetedCoupons.some((tId) => String(tId) === String(segmentId))
    ) {
      targetedSegments.push(index);
    }
  });

  // 2. Decide winning index
  ...
  // 3. Geometry calculation (correct + stable)
  ...
  let distanceToTravel =
    targetBaseAngle + randomOffset - currentRotation;

  if (distanceToTravel < 0) distanceToTravel += 360;
